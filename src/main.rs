...
use bme680::{Bme680, SettingsBuilder, OversamplingSetting, IIRFilterSize, PowerMode, I2CAddress, FieldDataCondition};
...
async fn main(spawner: Spawner) -> ! {
    ...
    let mut bmp688 = Bme680::init(i2c, &mut delayer, I2CAddress::Secondary).map_err(|e| {
    ...
    let b = Text::new(static_pressure_text, Point::new(static_pressure_x, static_pressure_y), text_style_new).draw(&mut display).unwrap();
    let c = Text::new(static_humidity_text, Point::new(static_humidity_x, static_humidity_y), text_style_new).draw(&mut display).unwrap();
    let d = Text::new(static_gas_text, Point::new(static_gas_x, static_gas_y), text_style_new).draw(&mut display).unwrap();
    println!("{:?}, {:?}, {:?}, {:?}", a, b, c, d);
    ...
    loop {
        ...
        if state == FieldDataCondition::NewData {
            if old_dyn_temp_text != data.temperature_celsius().to_string() {
                ...
                Text::new(&old_dyn_temp_text, Point::new(dyn_temp_x, dyn_temp_y), text_style_old).draw(&mut display).unwrap();
                Text::new(&dyn_temp_text, Point::new(dyn_temp_x, dyn_temp_y), text_style_new).draw(&mut display).unwrap();
            ...
            if old_dyn_pressure_text != data.pressure_hpa().to_string() {
                ...
                Text::new(&old_dyn_pressure_text, Point::new(dyn_pressure_x, dyn_pressure_y), text_style_old).draw(&mut display).unwrap();
                Text::new(&dyn_pressure_text, Point::new(dyn_pressure_x, dyn_pressure_y), text_style_new).draw(&mut display).unwrap();
            ...
            if old_dyn_humidity_text != data.humidity_percent().to_string() {
                ...
                Text::new(&old_dyn_humidity_text, Point::new(dyn_humidity_x, dyn_humidity_y), text_style_old).draw(&mut display).unwrap();
                Text::new(&dyn_humidity_text, Point::new(dyn_humidity_x, dyn_humidity_y), text_style_new).draw(&mut display).unwrap();
            ...
            if old_dyn_gas_text != data.gas_resistance_ohm().to_string() {
                ...
                Text::new(&old_dyn_gas_text, Point::new(dyn_gas_x, dyn_gas_y), text_style_old).draw(&mut display).unwrap();
                Text::new(&dyn_gas_text, Point::new(dyn_gas_x, dyn_gas_y), text_style_new).draw(&mut display).unwrap();
